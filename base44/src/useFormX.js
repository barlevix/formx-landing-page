 ...
 *
 * This is the standalone site's script.js — unchanged in substance, ported to a
 * hook: the same imperative DOM code, run once after the page mounts, with
 * teardown so it is safe across route changes and React's double-invoked
 * effects in development.
 *
 * Content comes from public/content.json. Media paths in that file are relative
 * (they must be, for the GitHub Pages copy), so they are resolved against the
 * site root here.
 */
...
export function useFormX() {
  useEffect(() => {
    ...
    const observers = [];
    const keep = (o) => { observers.push(o); return o; };
    ...
    async function loadContent() {
      try {
        ...
        const root = (p) => (typeof p === 'string' && p && !/^([a-z]+:)?\//i.test(p) ? '/' + p : p);
        if (data.hero) data.hero.src = root(data.hero.src), data.hero.poster = root(data.hero.poster);
        if (data.footer) data.footer.src = root(data.footer.src), data.footer.poster = root(data.footer.poster);
        (data.projects || []).forEach(p => {
          p.image = root(p.image);
          p.gallery = (p.gallery || []).map(root);
        });
        return {
    ...
    function setupTileReveal() {
      ...
      tileObserver = keep(new IntersectionObserver(entries => {
    ...
    setupTileReveal();
    DESKTOP.addEventListener('change', setupTileReveal, { signal });
    ...
    document.addEventListener('click', e => {
      const frame = e.target.closest('[data-project]', { signal });
    ...
    document.addEventListener('keydown', e => {
      if (e.key === 'Escape' && rail.classList.contains('is-open')) setSheet(false, { signal });
    });
    ...
    DESKTOP.addEventListener('change', onDesktopChange, { signal });
    ...
    function watchLazyVideos() {
      ...
      const io = keep(new IntersectionObserver((entries, obs) => {
        entries.forEach(e => {
